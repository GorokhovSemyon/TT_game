            ...
            for (int x = 0; x < FIELD_WIDTH; x++) {
                if (isBorderY(y)) {
                    ...
                    if (isBall(x, y, ball_x, ball_y)) {
                        ...
                        printf("%c", SYMBOL_MIDDLE);
                    } else if (isLeftPad(x, y, left_pad_y)) {
                        printf("%c", SYMBOL_LEFT_PAD);
                    } else if (isRightPad(x, y, right_pad_y)) {
                        printf("%c", SYMBOL_RIGHT_PAD);
                    } else {
                        printf("%c", SYMBOL_EMPTY_FIELD);
                    }
                }
            }
            printf("\n");
...
int isValidKey(char key) {
    return key == KEY_LEFT_UP
        || key == KEY_LEFT_DOWN
        || key == KEY_RIGHT_UP
        || key == KEY_RIGHT_DOWN
        || key == KEY_SKIP_STEP
        || key == KEY_LEFT_UP_UPPER
        || key == KEY_LEFT_DOWN_UPPER
        || key == KEY_RIGHT_UP_UPPER
        || key == KEY_RIGHT_DOWN_UPPER
            ? 1
            : 0;
}
...
void drawBorderWithPoints(int left_player_point, int right_player_point) {
    left_player_point >= 10 ? printf("%d", left_player_point) : printf("%d ", left_player_point);
    for (int i = 0; i < FIELD_WIDTH - 4; i++) {
        printf("%c", SYMBOL_BORDER);
    }
    right_player_point >= 10 ? printf("%d\n", right_player_point) : printf(" %d\n", right_player_point);
}
